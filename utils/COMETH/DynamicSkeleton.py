...
class DynamicSkeleton(ConstrainedSkeleton):
    def __init__(self, config=current_path+'BODY15_constrained_3D.xml', name=None, osim_file=None, geometry_dir='', max_velocity=None):
        ...
        self.RHip = (RASI+RPSI)/2 + np.array([0, -0.08, 0])
        self.LHip = (LASI+LPSI)/2 + np.array([0, -0.08, 0])

        # RClav = np.array([0,0,-0.01])
        # LClav = np.array([0,0,0.01])
        # self.Midshoulder = (RClav+LClav)/2    
        
        self.s12_base = Skeleton(current_path+'BODY12.xml')
        self.s15_base = Skeleton(current_path+'BODY15.xml')
        ...
        if  self.type == 'rajagopal':
            ...
            # Upper body: BODY15 (shkey)
            self.kps = ['Root', 'MidHip', 'MidShoulder', 'RShoulder', 'RElbow', 'RWrist', 'LShoulder', 'LElbow', 'LWrist', \
                                'RHip', 'RKnee', 'RAnkle', 'LHip', 'LKnee', 'LAnkle']
            nimble_joint_names = ['ground_pelvis', 'lumbar', 'thoracic_spine', 'GlenoHumeral_r', 'elbow_r', 'wrist_r', 'GlenoHumeral_l', 'elbow_l', 'wrist_l', \
                                'hip_r', 'walker_knee_r', 'ankle_r', 'hip_l', 'walker_knee_l', 'ankle_l']
            self.body_dict = {  'pelvis' :'Root', #'Core', #LPelvis
                                'femur_r':'RFemur',
                                'tibia_r':'RTibia',
                                'talus_r':'',
                                'calcn_r':'',
                                ...
                                'calcn_l':'',
                                'toes_l':'',
                                'lumbar_body':'Core', #'Core',#LClavicle
                                'thorax'    :'Core', #'Core',#LClavicle
                                'head'      :'',
                                'scapula_r' :'Core', #RClavicle',#LClavicle
                                'humerus_r' :'RHumerus',
                                'ulna_r'    :'RForearm',
                                'radius_r'  :'RForearm',
                                'hand_r'    :'',
                                'scapula_l' :'Core', #LClavicle',#LClavicle
                                'humerus_l' :'LHumerus',
                                'ulna_l'    :'LForearm',
                                'radius_l'  :'LForearm',
                                'hand_l'    :''}
        ...
        self.q_l[20], self.q_u[20] =  -5,  5    # lumbar_bending    -20, 20
        self.q_l[21], self.q_u[21] = -10, 30      # lumbar_extension    -5, 5
        self.q_l[22], self.q_u[22] = -10, 10      # lumbar_twist    -5, 5
        self.q_l[23], self.q_u[23] = -20, 10    # thorax_bending    -20, 20
        self.q_l[24], self.q_u[24] = -20, 10      # thorax_extension    -5, 5
        self.q_l[25], self.q_u[25] = -10, 10      # thorax_twist    -5, 5

        self.q_l[29], self.q_u[29] = -40, 30   # scapula_r_abduction   -90, -55
        self.q_l[30], self.q_u[30] = -30, 30      # scapula_r_elevation   -8, 2
        self.q_l[31], self.q_u[31] = -30, 30    # scapula_r_upward_rot  -10, 40
        self.q_l[32], self.q_u[32] = -180, 180    # shoulder_r_adduction   
        self.q_l[33], self.q_u[33] = -180, 180    # shoulder_r_flexion   
        self.q_l[34], self.q_u[34] = -180, 180   # shoulder_r_rotation    
        self.q_l[35], self.q_u[35] = -6, 150    # elbow_r_flexion       -6, 154
        self.q_l[36:38], self.q_u[36:38] = -90, 90   # wrist

        self.q_l[39], self.q_u[39] = -40, 30   # scapula_l_abduction   -90, -55
        self.q_l[40], self.q_u[40] = -30, 30      # scapula_l_elevation   -8, 2
        self.q_l[41], self.q_u[41] = -30, 30    # scapula_l_upward_rot  -10, 40
        self.q_l[42], self.q_u[42] = -180, 180     # shoulder_l_adduction  0, 150
        self.q_l[43], self.q_u[43] = -180, 180    # shoulder_l_flexion    -60, 180    
        self.q_l[44], self.q_u[44] = -180, 180   # shoulder_l_rotation    -90, 70
        self.q_l[45], self.q_u[45] = -6, 150    # elbow_l_flexion       -6, 154
        self.q_l[46:48], self.q_u[46:48] = -90, 90   # wrist
        ...
        self.s15_base.load_from_numpy(pos.reshape(-1,3),self.kps)
        self.skeleton_from_nimble.load_from_BODY15(self.s15_base)
    ...
    def correct(self,pos):
        ...
        # shkey: Commented out to avoid the correction of the scapula joints
